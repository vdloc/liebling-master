...
export function setupFeaturedSlider() {
  const featuredSliderElements = document.querySelectorAll(
    '.js-featured-slider'
  );

  if (featuredSliderElements.length > 0) {
    featuredSliderElements.forEach(elem => {
      const $elem = $(elem);
      const numSlides = $elem.find('.js-featured-slide').length;
      const featuredSlider = new Glide(elem, {
        type: 'slider',
        rewind: false,
        gap: 0,
      ...
      if (numSlides === 1) {
        $(elem)
          .find('.js-featured-slider-controls')
          .remove();
      }
